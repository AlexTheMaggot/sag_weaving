...
export default function MachineEdit(props) {
    ...
    return <MachineEditClass {...props} params={params}/>
}


class MachineEditClass extends Component {
    constructor(props) {
        super(props);
        this.state = {
            nav_auth: false,
    ...
    componentDidMount() {
        ...
        auth.then((response) => {
            if (response.error !== undefined) {
                ...
                let request = api_sender('MachineGetDetail', 1, {'id': this.props.params.machine_id})
                request.then((response) => {
                    if (response.error === undefined) {
                        this.setState({my_machine: response.result}, () => {
                            this.setState({name: this.state.my_machine.name}, () => {
                                setTimeout(() => {
    ...
    handleSubmit = (event) => {
        ...
        let request = api_sender('MachineUpdate', 2, {
            'id': this.state.my_machine.id,
    ...
    render() {
        return (
            <div className={this.state.show_content ? 'machineedit form__wrapper' : 'machineedit form__wrapper hidden'}>
                <form className="form" action="" method="POST" onSubmit={this.handleSubmit}>
                    <label className="form__label" htmlFor="name">Изменить название</label>
                    <input className="form__input" name="name" id="name" type="text" value={this.state.name} onChange={this.handleInput}/>
                    <div className="form__buttons">
                        <button className="form__button form__button_submit" type="submit">Изменить</button>
